...
export const sortGroups = (groups: Group[], _user: User) => {
  const personalOrgGroups = groups.filter(
    (_group) =>
      ...
      !_group.name.endsWith('Contributor') &&
      !_group.name.endsWith('Everyone'),
  );
  ...
  const otherGroups = groups.filter(
    (_group) =>
      ...
      (_group.organisation?.abbreviation === undefined || _group.organisation === null),
  );
...
export const sortGroupRoles = (userGroupRoles: GroupRole[], user: User) => {
  const personalOrgs = userGroupRoles
    .filter((group) => group.group.organisation?.abbreviation === user.orgAbbrev)
    .sort((a, b) => a.group.name.localeCompare(b.group.name));

  const foreignOrgs = userGroupRoles
    .filter(
      (group) =>
  ...
  const otherGroups = userGroupRoles
    .filter(
      (group) =>
        !personalOrgs.includes(group) &&
        !foreignOrgs.includes(group) &&
        (group.group.organisation?.abbreviation === undefined || group.group.organisation === null),
    )
    .sort((a, b) => a.group.name.localeCompare(b.group.name));
