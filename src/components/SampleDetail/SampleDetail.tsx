  ...
  useEffect(() => {
    const updateDisplayFields = async () => {
      ...
      if (response.status === 'Success') {
        ...
        setErrMsg(`Metadata fields for project ${projectAbbrev} could not be loaded`);
      }
    }
    if (project) {
      updateDisplayFields();
  ...
  useEffect(() => {
    const updateSampleData = async () => {
      ...
      if (response.status === 'Success' && response.data.length > 0) {
        ...
        setErrMsg(`Record ${seqId} could not be found within the context of project ${projectAbbrev}`);
      }
    }
    if (project) {
      updateSampleData();
