...
function PlotDetail() {
  ...
  const dataErrorMsg = useAppSelector((state) =>
    selectProjectMetadataError(state, projectAbbrev),
  );
  ...
  const renderPlot = () => {
    ...
    var props : PlotTypeProps = { projectAbbrev, customSpec: plot?.spec, setPlotErrorMsg: setErrorMsg };
