...
function ProFormaDetail() {
  ...
  useEffect(() => {
    ...
    const getProformas = async () => {
      ...
      if (proformaResponse.status === 'Success') {
        ...
        const keepVersions = versions
          .filter((v) => v.isCurrent || v.originalFileName !== null)
          ...
          {selectedVersion?.originalFileName ? (
            <Stack direction="row" spacing={1}>
              ...
              <Stack direction="column" width="calc(100% - 40px)">
                <Typography gutterBottom>Download latest Excel pro forma template</Typography>
                <Typography
  ...
  return errMsg ? (
    ...
    <>
      ...
      <Typography sx={{ mb: '2em' }}>{proforma?.description}</Typography>
      ...
      <Select
        ...
        onChange={(e) => {
          setSelectedVersion(
            proformaVersionList.find((v) => v.version.toString() === e.target.value),
          );
        }}
      >
      ...
      <Typography sx={{ mt: 3 }}>
        ...
        against field types and values. &quot;Strictly required&quot; fields cannot be left blank in
        any upload. Some fields which are not strictly required, and which may be temporarily left
        blank, may still form part of the minimum expected metadata set for a project.
      </Typography>
      <TableContainer
        component={Paper}
        sx={{ mt: 3, mb: 4, ml: 'auto', mr: 'auto', maxWidth: '60em' }}
      >
        <Table>
          <TableHead>
            <TableRow>
            ...
            {selectedVersion?.columnMappings.map((row) => (
              <TableRow key={row.metaDataColumnName}>
                ...
                <TableCell align="right">
                  {row.metaDataColumnPrimitiveType === null
                    ? 'categorical'
                    : row.metaDataColumnPrimitiveType}
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
