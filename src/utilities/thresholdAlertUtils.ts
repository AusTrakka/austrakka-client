...
export const calculateAlert = (
  ...
  const dedupedMap = data.reduce((acc, row) => {
  ...
  if (recentCount === 0) {
    ...
    alertLevel = OrderedAlertLevels.slice()
      .reverse()
      .find((level) => ratio! > AlertThresholds[level])!;
  }
  return {
...
export const calculateAlertList = (
  ...
  const filteredData = data.filter((row) =>
    requiredFields.every((field) => !isNullOrEmpty(row[field])),
  );
